...
import { createProjectPlaceApi } from './apis/projectplace';
...
export const Config = () => {
    const refs = useRef({
        ...
        appKeyInput: undefined as HTMLInputElement,
        appSecretInput: undefined as HTMLInputElement,
        accessTokenKeyInput: undefined as HTMLInputElement,
        accessTokenSecret: undefined as HTMLInputElement,
    ...
    useAsyncEffect(async ({ aborted }) => { // load settings from local storage
        ...
        if (settings?.pp_timer) return;
        refs.current.appKeyInput?.focus(); // focus on base Url input
        setHelp('Application Key');
    }, []);
    ...
    const { pp_timer: { appKey } = {}, theme: { isDark, lineHeight } = {}, numberOfDays, workHours: [workHoursStart, workHoursEnd], skipAnimation, autoRefresh, hideInactive } = settings ?? defaultSettings;
    const propsBaseUrlInput = {
        ref: (ref: HTMLInputElement) => refs.current.appKeyInput = ref,
        name: 'BaseUrl', placeholder: 'Step 1 > Redmine URL', defaultValue: appKey, disabled: !!appKey,
